...
| `BROWSERMESH_SUPPORT_REQUEST`              | `true` (attended) | exact `true` or `false`                    |
...
`BROWSERMESH_SUPPORT_REQUEST` also defaults to `false` when the server process sees `CI`. Treat that
as best-effort rather than a guarantee: an MCP client starts a stdio server with a minimal
environment — the reference transport forwards only `HOME`, `LOGNAME`, `PATH`, `SHELL`, `TERM`, and
`USER` — so `CI` usually does not reach the server even when it is set in the surrounding job. Set
the variable explicitly where it matters.
