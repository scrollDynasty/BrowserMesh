...
| Bounded snapshots, context options, typed interactions | Partial: bounded scoped snapshots, safe context settings, short-lived refs, hover/focus/check/double-click/scroll/scroll-into-view/drag-drop, viewport/full-page/element in-memory screenshots, popup/dialog, and iframe-scoped semantic targeting complete; tree filters and remaining lifecycle-heavy interactions pending | ADRs 0011, 0013, 0014 |
...
- ADR 0014 adds engine-neutral main-document or one-through-five-step iframe scopes to every
  ...
  navigation/detach.
- Iframe-targeting `npm run verify`: passed (23 files, 128 tests, coverage thresholds, lint,
  formatting, typecheck, and build). `BROWSERMESH_HEADLESS=true npm run verify:package`: passed,
  including packed-file inspection, clean tarball installation, MCP discovery, real Chromium
  lifecycle smoke, and shutdown.
